  ...
  begin
    if new_resource.support_os.include?($gecos_os)
      ...
      users.each_key do |user_key|
        ...
        # If user has been created but hasn't log in to his/her desktop,
        # .config/gtk-3.0 directory will be missing
        ...
        gtkbookmark_files.each do |gtkbook|
          ...
          user.gtkbookmarks.each do |bookmark|
            if bookmark.uri.match(pattern)
              ...
              # Do not create the bookmark if the variable expansion
              # returns nil
              if bookmark_uri && bookmark_name
                line_to_add = "#{bookmark_uri} #{bookmark_name}"
                # If there's no line containing the bookmark URI
                # (removing lading spaces and trailing slash),
                # insert the bookmark.
              ...
              Chef::Log.warn('Bookmark URI doesn\'t match the pattern:'\
                  " #{bookmark.uri} username: #{username}")
            end
          end
          tmp_file.write_file
        end
